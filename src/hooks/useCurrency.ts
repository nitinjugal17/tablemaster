...
import { useGeneralSettings } from '@/context/GeneralSettingsContext'; // Import new context
...
export function useCurrency() {
  ...
  const [displayCurrencySymbol, setDisplayCurrencySymbol] = useState<CurrencySymbol>(defaultInvoiceSetupSettings.currencySymbol);
  const [displayCurrencyCode, setDisplayCurrencyCode] = useState<CurrencyCode>(defaultInvoiceSetupSettings.currencyCode);
  
  useEffect(() => {
    if (!isLoadingGeneralSettings && generalSettings) {
      setDisplayCurrencySymbol(generalSettings.currencySymbol || defaultInvoiceSetupSettings.currencySymbol);
      setDisplayCurrencyCode(generalSettings.currencyCode || defaultInvoiceSetupSettings.currencyCode);
    }
