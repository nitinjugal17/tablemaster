...
const MenuItemCard: React.FC<MenuItemCardProps> = ({ item, onAddToCart, isDiscounted, addonGroups }) => {
  ...
  const defaultPortion = useMemo(() => {
    const df = item.portionDetails?.find(p => p.isDefault);
    return df || item.portionDetails?.[0] || { name: "fixed", price: 0, isDefault: true };
  }, [item.portionDetails]);
  ...
  const hasMultiplePortions = item.portionDetails && item.portionDetails.length > 1;
                ...
                {hasMultiplePortions ? (
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" onClick={(e) => e.stopPropagation()}>
                        {item.portionDetails.map(portion => (
      ...
      <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col p-0">
              ...
              <Card>
                ...
                <CardContent>
                  <RadioGroup value={selectedPortion.name} onValueChange={(value) => {
                    const newPortion = item.portionDetails.find(p => p.name === value) || defaultPortion;
                    ...
                    {item.portionDetails.map(portion => (
                      <div key={portion.name} className="flex justify-between items-center p-2 rounded-md hover:bg-muted/50">
                        ...
                        <span className="font-semibold">{currencySymbol}{convertPrice(portion.price).toFixed(2)}</span>
                      </div>
                    ))}
                  </RadioGroup>
                </CardContent>
                            ...
                            <div key={group.id}>
                                ...
                                {group.addons.map(addon => (
                                    <div key={addon.id} className="flex items-center space-x-2 py-1">
                                        ...
                                        <span className="text-sm text-muted-foreground">+ {currencySymbol}{convertPrice(addon.price).toFixed(2)}</span>
                                    </div>
                                ))}
                            </div>
                        ))}
        ...
        <DialogFooter className="p-4 border-t flex-shrink-0 sm:justify-between items-center">
            <div className="text-xl font-bold text-accent">
                {currencySymbol}{convertPrice(selectedPortion.price).toFixed(2)}
            </div>
