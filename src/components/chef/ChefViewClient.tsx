...
import { updateOrderStatus, getPendingKOTQueue, printPendingKOTs } from '@/app/actions/order-actions'; 
...
export function ChefViewClient({ initialOrders, initialPrinters }: ChefViewClientProps) {
  ...
  const fetchAndSetOrders = useCallback(async (isAutoRefresh = false) => {
    ...
    try {
      const [fetchedOrders, fetchedPrinters, pendingKots] = await Promise.all([
        getOrders(), getPrinterSettings(), getPendingKOTQueue()
      ]);
      setOrders(fetchedOrders); 
      setPrinters(fetchedPrinters);
      setPendingKotCount(pendingKots.length);
  ...
  // Also fetch KOT count on initial load
  useEffect(() => {
    getPendingKOTQueue().then((pendingKots: Order[]) => setPendingKotCount(pendingKots.length));
  }, []);
  ...
  const handlePrintThermal = async (order?: Order, allOrders?: Order[]) => {
    ...
    const defaultPrinterId = generalSettings.defaultChefKotPrinterId || generalSettings.defaultThermalPrinterId;
    const printerToUse = printers.find(p => p.id === defaultPrinterId);
    if (!printerToUse) {
      toast({ title: "Printer Not Configured", description: "No default Chef or Main thermal printer is set in Invoice Settings.", variant: "destructive" });
      return;
    }
  ...
  const handleManualBatchPrint = async () => {
    setIsPrinting(true);
    const result = await printPendingKOTs();
    if (result.success) {
      toast({ title: "Printing Queued KOTs", description: result.message });
      await fetchAndSetOrders(); // Refresh to update count
    } else {
  ...
  const configuredPrinterName = useMemo(() => {
    ...
    const chefPrinter = printers.find(p => p.id === chefPrinterId);
    if (chefPrinter) return `${chefPrinter.name} (Chef KOT)`;

    const mainPrinter = printers.find(p => p.id === mainPrinterId);
    if (mainPrinter) return `${mainPrinter.name} (Default)`;
    
    return "Not Configured";
  }, [generalSettings, isLoadingSettings, printers]);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2 items-center w-full sm:w-auto">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
                {pendingKotCount > 0 && generalSettings?.autoKotPrintMode === 'batch' && (
                    <Button onClick={handleManualBatchPrint} variant="destructive" size="sm" disabled={isLoading || isPrinting} className="flex-1 relative">
                        <PrinterIcon className="mr-2 h-4 w-4" /> 
                        Print Pending ({pendingKotCount})
                        <span className="absolute -top-1 -right-1 flex h-3 w-3"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-destructive opacity-75"></span><span className="relative inline-flex rounded-full h-3 w-3 bg-red-600"></span></span>
                    </Button>
                )}
