...
const ChefOrderCard: React.FC<ChefOrderCardProps> = ({ order, preparedItems, onItemToggle, onMarkOrderReady, isUpdatingStatus }) => {
  const allItemsPrepared = order.items.every((item, index) => preparedItems[getItemKey(order.id, item.menuItemId, index, item.selectedPortion)]);
  const timeSincePlaced = useMemo(() => {
    try {
      return formatDistanceToNow(parseISO(order.createdAt), { addSuffix: true });
    } catch (e) {
      return "Unknown time";
    }
  }, [order.createdAt]);
  ...
  return (
    <Card className="shadow-lg">
      <CardHeader>
        ...
        <CardDescription className="text-xs">Placed: {timeSincePlaced} ({order.customerName})</CardDescription>
      </CardHeader>
      <CardContent>
