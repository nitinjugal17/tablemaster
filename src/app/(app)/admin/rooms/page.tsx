...
export default function RoomManagementPage() {
  ...
  const handleSaveRoom = (roomData: Room) => {
    setRooms(prevRooms => {
      const existingIndex = prevRooms.findIndex(r => r.id === roomData.id);
      if (existingIndex > -1) {
    ...
    toast({ title: "Room Updated Locally", description: `${roomData.name} has been updated. Save all to persist.` });
  ...
  const handleSaveAllToDataSource = async () => {
    setIsSaving(true);
    try {
      const result = await saveRooms(rooms);
      if (result.success) {
        toast({ title: "Rooms Saved!", description: result.message });
      } else {
        toast({ title: "Error Saving Rooms", description: result.message, variant: "destructive" });
      }
    } catch (error) {
      toast({ title: "Error Saving Rooms", description: "An unexpected error occurred.", variant: "destructive" });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDeleteRoom = (roomId: string) => {
    ...
    toast({ title: "Room Marked for Deletion", description: "Save all to persist.", variant: "destructive" });
  };
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <Button onClick={handleSaveAllToDataSource} disabled={isSaving || isLoading}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Overview of all bookable rooms. Changes are temporary until "Save All Changes" is clicked.</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                          ...
                          <AlertDialog> 
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  This will mark the room "{room.name}" for deletion.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
                                <AlertDialogCancel>Cancel</AlertDialogCancel>
                                <AlertDialogAction onClick={() => handleDeleteRoom(room.id)}>Delete Locally</AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
                        </DropdownMenuContent>
