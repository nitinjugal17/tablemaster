...
import type { Order, PaymentType, MenuItem as MenuItemType, PrinterSetting, InvoiceSetupSettings, PrintableInvoiceData, AppLanguage, OrderStatus, OrderType } from '@/lib/types';
...
import {
  ...
  LayoutGrid, List, XCircle, Truck, AlertCircle, Printer as PrinterIcon, FileText as FileTextIcon, Mail as MailIcon, BadgePercent
} from 'lucide-react';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuTrigger } from "@/components/ui/dropdown-menu";
import { format, parseISO, isToday, isWithinInterval, startOfDay, endOfDay, isValid } from 'date-fns';
import { getOrders, getMenuItems, getPrinterSettings } from '@/app/actions/data-management-actions';
...
export default function AdminOrdersPage() {
  ...
  useEffect(() => {
    async function fetchAdminPageData() {
      setIsLoading(true);
      try {
        const [fetchedOrders, fetchedMenuItems, fetchedAllPrinters] = await Promise.all([
          getOrders(),
          getMenuItems(),
          getPrinterSettings()
        ]);
        ...
        console.error("Failed to fetch admin orders, menu items, or printer settings:", error);
        toast({
  ...
  const handleSaveOrder = (updatedOrderData: Partial<Order>) => {
     setOrders(prevOrders =>
        prevOrders.map(o => o.id === updatedOrderData.id ? {...o, ...updatedOrderData} as Order : o)
     );
     toast({ title: "Order Saved (Locally)", description: `Order ${updatedOrderData.id ? String(updatedOrderData.id).substring(0,8) : 'N/A'} has been updated. Not persisted to CSV yet.` });
  ...
  const handleOpenInvoiceViewer = async (order: Order) => {
    if (isLoadingGeneralSettings || isLoading) {
  ...
  const handlePrintThermal = async () => {
    ...
    if (printerToUse.connectionType === 'system') {
        ...
        handlePrintWebPreview('invoice-preview-content-thermal-for-os', 'thermal'); 
        return;
    }
  ...
  const handleEmailReceipt = async () => {
    ...
    try {
        const result = await sendInvoiceEmail({ invoiceData: { ...printableInvoiceData, customerEmail }});
        if (result.success) {
            toast({
                title: "Receipt Emailed",
                description: `Invoice for order #${String(printableInvoiceData.order.id).substring(0,8)} sent to ${customerEmail}. ${result.messageId === 'mock_message_id' ? '(Mocked)' : ''}`,
            });
        } else {
    ...
    return orders.filter(order => {
      ...
      if (order.createdAt) {
        try {
          ...
          console.warn(`[AdminOrdersPage] Error parsing date for order ${order.id}: "${order.createdAt}"`, e);
          dateMatch = !dateRange.from && !dateRange.to;
        }
      } else {
         dateMatch = !dateRange.from && !dateRange.to;
  ...
  return (
    <div className="space-y-8">
       <Dialog open={isEditorOpen} onOpenChange={(open) => { setIsEditorOpen(open); if (!open) setEditingOrder(undefined);}}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] flex flex-col">
          ...
          {isEditorOpen && editingOrder && <OrderEditor order={editingOrder} menuItems={menuItems} onSave={handleSaveOrder} />}
        </DialogContent>
      </Dialog>

      <Dialog open={isInvoiceViewerOpen} onOpenChange={(open) => { setIsInvoiceViewerOpen(open); if(!open) setViewingInvoiceOrder(undefined); }}>
          ...
          <div className="px-6 py-3 border-b bg-muted/50 space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                <div className="space-y-1">
                  <Label htmlFor="print-discount-type" className="text-xs">Discount Type</Label>
                ...
                {printTimeDiscountType !== 'none' && (
                  <div className="space-y-1">
                    <Label htmlFor="print-discount-value" className="text-xs">Discount Value</Label>
                ...
                <>
                    ...
                    <div style={{ display: 'none' }}>
                        <InvoicePreview
                            data={invoicePreviewData}
                            previewType="thermal"
                            id="invoice-preview-content-thermal-for-os"
                        />
                    </div>
                </>
            ) : (
        ...
        {isLoading ? (
            ...
            <>
                ...
                <StatCard title="Completed Today" value={orders.filter(o => {
                    try { return o.status === 'Completed' && isValid(parseISO(o.createdAt)) && isToday(parseISO(o.createdAt)); }
                    catch { return false; }
                    ...
                    <ScrollArea className="h-[calc(100vh-25rem)]">
                        ...
                        {filteredOrders.map(order => (
                            <Card key={order.id} className="shadow-md hover:shadow-lg transition-shadow flex flex-col">
                                <OrderCard order={order} isAdminView={true} />
                            ...
                            <TableBody>
                                {filteredOrders.map(order => (
                                    <TableRow key={order.id}>
                                        <TableCell className="font-medium text-primary">#{String(order.id).substring(0,8)}...</TableCell>
                                        <TableCell>{order.customerName}</TableCell>
                                        <TableCell>
                                            {order.createdAt && isValid(parseISO(order.createdAt))
                                                ? format(parseISO(order.createdAt), "MMM d, yy, h:mm a")
                                                : "Invalid Date"}
                                        </TableCell>
