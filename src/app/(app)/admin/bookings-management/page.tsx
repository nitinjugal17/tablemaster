...
import { updateBookingDetails, saveNewBooking, sendBookingConfirmationEmail } from '@/app/actions/booking-actions';
...
import {
    Loader2, MoreVertical, SlidersHorizontal, CalendarDays, Users, Columns3, PackageSearch,
    CheckCircle, XCircle, ClockIcon, SquarePen, Trash2, PlusCircle, ShoppingCart, Utensils, StickyNote, MessageSquare, Search, ChevronDown, Timer, RefreshCw, BedDouble
} from 'lucide-react';
...
export default function AdminBookingsManagementPage() {
  ...
  const [selectedMenuItemForQuickAdd, setSelectedMenuItemForQuickAdd] = useState<string>("");
  const [quantityForQuickAdd, setQuantityForQuickAdd] = useState<number>(1);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setBookings(fetchedBookings.sort((a,b) => parseISO(b.date).getTime() - parseISO(a.date).getTime()));
  ...
  const uniqueCategoriesForDialog = useMemo(() => [...new Set(menuItems.map(item => item.category).filter(Boolean).sort())], [menuItems]);
  const uniqueCuisinesForDialog = useMemo(() => ['all', ...new Set(menuItems.map(item => item.cuisine).filter((c): c is string => !!c).sort())], [menuItems]);
  const uniqueDietaryOptionsForDialog = useMemo(() => ['all', ...new Set(menuItems.map(item => item.dietaryRestrictions).filter((c): c is string => !!c).sort())], [menuItems]);
  ...
  const filteredMenuItemsForDialog = useMemo(() => {
    return menuItems.filter(item => {
      ...
      const searchFields = [item.name.toLowerCase(), item.id.toLowerCase(), item.category.toLowerCase(), item.cuisine?.toLowerCase() || '', item.ingredients?.toLowerCase() || '', item.synonyms?.toLowerCase() || '', item.description?.toLowerCase() || '',];
  ...
  const handleConfirmStatusUpdate = async () => {
    ...
    if (result.success && result.updatedBooking) {
      setBookings(prev => prev.map(b => b.id === bookingForAction.id ? result.updatedBooking! : b));
  ...
  }, [bookings, searchTerm, filterValues]);
  ...
  const handleQuickAddItemToCart = () => {
    if (!selectedMenuItemForQuickAdd || quantityForQuickAdd < 1) { toast({ title: "Select Item", description: "Please select a menu item and quantity.", variant: "destructive" }); return; }
    const menuItem = menuItems.find(mi => mi.id === selectedMenuItemForQuickAdd);
    if (!menuItem) return;
    const defaultPortion = menuItem.portionDetails.find(p => p.isDefault) || menuItem.portionDetails[0];
    if (!defaultPortion) { toast({ title: "Item Error", description: `Item "${menuItem.name}" has no defined portions/price. Cannot add to cart.`, variant: "destructive"}); return; }
    const priceToAdd = defaultPortion.price; const portionNameToAdd = defaultPortion.name;
    setQuickBookingCart(prev => {
      const existingItemIndex = prev.findIndex(item => item.menuItemId === menuItem.id && item.selectedPortion === portionNameToAdd);
      if (existingItemIndex > -1) { const updatedCart = [...prev]; updatedCart[existingItemIndex].quantity += quantityForQuickAdd; return updatedCart; }
      return [...prev, { menuItemId: menuItem.id, name: menuItem.name, price: priceToAdd, quantity: quantityForQuickAdd, selectedPortion: portionNameToAdd, note: "" }];
    });
    setSelectedMenuItemForQuickAdd(""); setQuantityForQuickAdd(1);
  };
  const handleQuickUpdateQuantity = (itemId: string, portion: string | undefined, newQuantity: number) => { setQuickBookingCart(prev => prev.map(item => (item.menuItemId === itemId && item.selectedPortion === portion) ? { ...item, quantity: newQuantity } : item).filter(item => item.quantity > 0)); };
  ...
  const resetQuickCreateDialog = () => {
    quickBookingForm.reset({ 
        ...
        time: (() => { const now = new Date(); let h=getHours(now); let m=getDateMinutes(now); if(m > 0 && m<=30) m=30; else if(m > 30) {m=0; h=(h+1)%24;} return `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`; })(),
        bookingType: 'table', assignedResourceId: "", notes: "", preOrderItems: [],
    });
    setQuickBookingCart([]); clearPreOrderFiltersDialog(); setSelectedMenuItemForQuickAdd(""); setQuantityForQuickAdd(1);
  };
  ...
  async function onQuickBookingSubmit(values: QuickBookingFormValues) {
    ...
    if (result.success && result.bookingId) {
        ...
        setBookings(prev => [newBooking, ...prev].sort((a,b) => parseISO(b.date).getTime() - parseISO(a.date).getTime()));
        if (newBooking.email) { await sendBookingConfirmationEmail(newBooking, displayCurrencyCode, currencySymbol); }
        setIsQuickCreateDialogOpen(false); resetQuickCreateDialog();
    } else {
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div><CardTitle className="font-headline">Booking List ({bookings.length})</CardTitle><CardDescription>Filter and manage customer bookings.</CardDescription></div>
            ...
            <ScrollArea className="h-[calc(100vh-20rem)]"><Table><TableHeaderComponent><TableRow>
              ...
              {filteredBookings.map(booking => {
                ...
                return ( <TableRow key={booking.id}>
                    ...
                    <TableCell>{booking.items && booking.items.length > 0 ? <Badge variant="outline">{booking.items.reduce((sum, item) => sum + item.quantity, 0)} items</Badge> : <Badge variant="secondary">No</Badge>}</TableCell>
                    ...
                    <TableCell className="text-right"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon"><MoreVertical className="h-5 w-5" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end">
                        ...
                        {booking.status === 'pending' && <DropdownMenuItem onClick={() => handleOpenStatusUpdateDialog(booking, 'confirmed')} disabled={isUpdatingBooking}><CheckCircle className="mr-2 h-4 w-4"/> Confirm & Assign Resource</DropdownMenuItem>}
                        {booking.status === 'confirmed' && <DropdownMenuItem onClick={() => handleOpenStatusUpdateDialog(booking, 'confirmed')} disabled={isUpdatingBooking}><SquarePen className="mr-2 h-4 w-4"/> Change Assigned Resource</DropdownMenuItem>}
                        ...
                        <DropdownMenuSeparator /><DropdownMenuItem disabled><Trash2 className="mr-2 h-4 w-4"/> Delete (Not Impl.)</DropdownMenuItem>
            ...
            {targetStatusForUpdate === 'confirmed' && (
              <div>
                ...
                <Select value={resourceToAssign || ''} onValueChange={setResourceToAssign}>
                  ...
                  <SelectContent>
                    {bookingForAction?.bookingType === 'table' ? availableResourcesForAssignment.tables.map(table => ( <SelectItem key={table.id} value={table.id} disabled={table.status !== 'Available' && table.id !== bookingForAction?.assignedResourceId && table.id !== bookingForAction?.requestedResourceId}>{table.name} (Capacity: {table.capacity}) {table.status !== 'Available' && `(${table.status})`}</SelectItem> )) : 
                     availableResourcesForAssignment.rooms.map(room => ( <SelectItem key={room.id} value={room.id}>{room.name} (Capacity: {room.capacity})</SelectItem> ))
                    }
                    {bookingForAction?.bookingType === 'table' && availableResourcesForAssignment.tables.length === 0 && <SelectItem value="no-resources" disabled>No suitable tables</SelectItem>}
                    {bookingForAction?.bookingType === 'room' && availableResourcesForAssignment.rooms.length === 0 && <SelectItem value="no-resources" disabled>No suitable rooms</SelectItem>}
                  </SelectContent>
                </Select>
                <p className="text-xs text-muted-foreground mt-1">Only available or already associated resources are shown.</p>
              </div>
            )}
            <div>
              <Label htmlFor="adminNotes">Notes for Customer (Optional)</Label>
              <Textarea id="adminNotes" value={adminNotes} onChange={(e) => setAdminNotes(e.target.value)} placeholder={targetStatusForUpdate === 'cancelled' ? 'Reason for cancellation...' : 'e.g., Special arrangements have been made.'}/>
              <p className="text-xs text-muted-foreground mt-1">These notes will be included in the notification email to the customer and added to the booking history.</p>
            </div>
          </div>
          <DialogFooter>
              <DialogClose asChild><Button variant="outline">Cancel</Button></DialogClose>
              <Button onClick={handleConfirmStatusUpdate} disabled={isUpdatingBooking}>
                  {isUpdatingBooking && <Loader2 className="mr-2 h-4 w-4 animate-spin"/>}
        ...
        <form onSubmit={quickBookingForm.handleSubmit(onQuickBookingSubmit)} className="space-y-4 max-h-[80vh] overflow-y-auto p-1 pr-2">
            ...
            <Card className="pt-3"><CardHeader className="p-2 pb-1"><CardTitle className="text-base font-semibold flex items-center"><Utensils className="mr-2 h-4 w-4 text-accent"/>Pre-order Items (Optional)</CardTitle></CardHeader><CardContent className="p-2 space-y-2">
                <div className="flex flex-col sm:flex-row gap-2 items-center">
                    <div className="relative w-full sm:flex-grow"><Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input type="search" placeholder="Search items..." value={menuSearchTermDialog} onChange={(e) => setMenuSearchTermDialog(e.target.value)} className="pl-8 h-9 text-sm"/></div>
                    <DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" size="sm" className="w-full sm:w-auto justify-between h-9 text-sm">{getCategoryButtonTextDialog()}<ChevronDown className="ml-2 h-4 w-4 opacity-50"/></Button></DropdownMenuTrigger><DropdownMenuContent className="w-56 max-h-60 overflow-y-auto"><DropdownMenuCheckboxItem checked={selectedCategoriesDialog.length === 0} onCheckedChange={() => setSelectedCategoriesDialog([])}>All Categories</DropdownMenuCheckboxItem><DropdownMenuSeparator />{uniqueCategoriesForDialog.map(cat => ( <DropdownMenuCheckboxItem key={cat} checked={selectedCategoriesDialog.includes(cat)} onCheckedChange={() => handleCategoryChangeDialog(cat)} className="capitalize">{cat}</DropdownMenuCheckboxItem> ))}</DropdownMenuContent></DropdownMenu>
                    <Popover><PopoverTrigger asChild><Button variant="outline" size="icon" className="h-9 w-9 shrink-0"><SlidersHorizontal className="h-4 w-4"/></Button></PopoverTrigger><PopoverContent className="w-64 z-50" align="end"><div className="grid gap-3"><div className="space-y-1"><h4 className="font-medium leading-none text-xs">More Item Filters</h4></div><div className="grid gap-2"><div className="grid grid-cols-3 items-center gap-2"><Label htmlFor="filterCuisineDialog" className="text-xs">Cuisine</Label><Select value={filterCuisineDialog} onValueChange={setFilterCuisineDialog}><SelectTrigger id="filterCuisineDialog" className="col-span-2 h-7 text-xs"><SelectValue /></SelectTrigger><SelectContent>{uniqueCuisinesForDialog.map(c => <SelectItem key={c} value={c} className="capitalize text-xs">{c === 'all' ? 'All' : c}</SelectItem>)}</SelectContent></Select></div><div className="grid grid-cols-3 items-center gap-2"><Label htmlFor="filterDietaryDialog" className="text-xs">Dietary</Label><Select value={filterDietaryDialog} onValueChange={setFilterDietaryDialog}><SelectTrigger id="filterDietaryDialog" className="col-span-2 h-7 text-xs"><SelectValue /></SelectTrigger><SelectContent>{uniqueDietaryOptionsForDialog.map(opt => <SelectItem key={opt} value={opt} className="capitalize text-xs">{opt === 'all' ? 'All' : opt}</SelectItem>)}</SelectContent></Select></div></div><Button onClick={clearPreOrderFiltersDialog} variant="ghost" size="sm" className="w-full justify-start text-xs text-muted-foreground hover:text-primary">Clear Item Filters</Button></div></PopoverContent></Popover>
                </div>
                <div className="flex gap-2 items-end mt-2">
                    <div className="flex-grow"><Label htmlFor="quick-add-item" className="text-xs sr-only">Menu Item</Label><Select value={selectedMenuItemForQuickAdd} onValueChange={setSelectedMenuItemForQuickAdd}><SelectTrigger id="quick-add-item" className="h-9 text-sm"><SelectValue placeholder={filteredMenuItemsForDialog.length > 0 ? "Select item" : "No items match filters"}/></SelectTrigger><SelectContent>{filteredMenuItemsForDialog.map(mi => { const defaultPortion = mi.portionDetails.find(p => p.isDefault) || mi.portionDetails[0] || { name: "fixed", price: 0 }; const priceToDisplay = defaultPortion.price; const displayPortionName = defaultPortion.name; const hasMultiplePortions = mi.portionDetails.length > 1; const itemDisplayName = hasMultiplePortions && displayPortionName !== "fixed" ? `${mi.name} (${displayPortionName})` : mi.name; return (<SelectItem key={mi.id} value={mi.id} className="text-sm">{itemDisplayName} ({currencySymbol}{convertPrice(priceToDisplay).toFixed(2)})</SelectItem>);})}{filteredMenuItemsForDialog.length === 0 && <div className="p-2 text-center text-xs text-muted-foreground">No items found.</div>}</SelectContent></Select></div>
                    <div className="w-20"><Label htmlFor="quick-add-qty" className="text-xs sr-only">Qty</Label><Input id="quick-add-qty" type="number" value={quantityForQuickAdd} onChange={e => setQuantityForQuickAdd(Number(e.target.value))} min="1" className="h-9 text-sm"/></div>
                    <Button type="button" variant="outline" size="sm" onClick={handleQuickAddItemToCart} disabled={!selectedMenuItemForQuickAdd} className="h-9"><PlusCircle className="mr-1 h-4 w-4"/>Add</Button>
                </div>
                {quickBookingCart.length > 0 && (<ScrollArea className="max-h-32 border rounded-md p-2 mt-2"><div className="space-y-1.5">
                {quickBookingCart.map((item, index) => { const itemDisplayName = item.selectedPortion && item.selectedPortion !== "fixed" ? `${item.name} (${item.selectedPortion})` : item.name; return ( <div key={`${item.menuItemId}-${item.selectedPortion || 'fixed'}-${index}`} className="flex items-start justify-between text-xs gap-2"><div className="flex-grow space-y-1"><span className="font-medium block">{itemDisplayName}</span><Input type="text" placeholder="Item specific note (e.g. no onion)" value={item.note || ""} onChange={(e) => { const newCart = [...quickBookingCart]; newCart[index].note = e.target.value; setQuickBookingCart(newCart);}} className="h-7 text-xs w-full"/></div><div className="flex items-center gap-1 shrink-0"><Input type="number" value={item.quantity} onChange={e => handleQuickUpdateQuantity(item.menuItemId, item.selectedPortion, Number(e.target.value))} className="w-12 h-6 text-xs text-center" min="1"/><Button type="button" variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => handleQuickUpdateQuantity(item.menuItemId, item.selectedPortion, 0)}><Trash2 className="h-3 w-3"/></Button></div></div>)})}</div></ScrollArea>)}
