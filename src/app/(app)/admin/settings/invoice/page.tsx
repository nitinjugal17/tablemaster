...
export default function InvoiceSettingsPage() {
  ...
  // State for KOT settings
  const [kotSettings, setKotSettings] = useState({
      defaultChefKotPrinterId: generalSettings.defaultChefKotPrinterId || '__NONE__',
      chefKotLanguage: generalSettings.chefKotLanguage || 'en',
      defaultStockKotPrinterId: generalSettings.defaultStockKotPrinterId || '__NONE__',
      stockKotLanguage: generalSettings.stockKotLanguage || 'en',
      enableAutoKotPrinting: generalSettings.enableAutoKotPrinting || false,
      autoKotPrintMode: generalSettings.autoKotPrintMode || 'immediate',
      kotBatchPrintThreshold: generalSettings.kotBatchPrintThreshold || 2
  });
  ...
  const handleKOTSettingChange = async () => {
    ...
    try {
        const updatedGeneralSettings = {
            ...generalSettings,
            ...kotSettings,
            defaultChefKotPrinterId: kotSettings.defaultChefKotPrinterId === '__NONE__' ? undefined : kotSettings.defaultChefKotPrinterId,
            defaultStockKotPrinterId: kotSettings.defaultStockKotPrinterId === '__NONE__' ? undefined : kotSettings.defaultStockKotPrinterId,
        };
        const result = await saveGeneralSettingsAction(updatedGeneralSettings);
        if (result.success) {
            await refreshGeneralSettings();
            toast({ title: "KOT Settings Saved", description: "The KOT configuration has been updated."});
        } else {
             toast({ title: "Error Saving Settings", description: result.message, variant: "destructive" });
        }
    } catch(e) {
        toast({ title: "Error", description: "Could not save KOT settings.", variant: "destructive"});
    } finally {
        setIsSettingDefault(false);
    }
  };
  ...
  const handleTestPrinter = async () => {
    ...
    if (currentEditingPrinter.connectionType !== 'network') {
        ...
        toast({ title: "Informational", description: message, duration: 10000 });
        return;
    }
            ...
            <CardContent className="space-y-4">
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        ...
                        <Select
                            value={kotSettings.defaultChefKotPrinterId}
                            onValueChange={(val) => setKotSettings(prev => ({...prev, defaultChefKotPrinterId: val}))}
                    ...
                    <div className="space-y-2">
                        ...
                        <Select
                            value={kotSettings.chefKotLanguage}
                            onValueChange={(val) => setKotSettings(prev => ({...prev, chefKotLanguage: val as AppLanguage}))}
                    ...
                    <div className="space-y-2">
                        ...
                        <Select
                            value={kotSettings.defaultStockKotPrinterId}
                            onValueChange={(val) => setKotSettings(prev => ({...prev, defaultStockKotPrinterId: val}))}
                            disabled={isSettingDefault || printers.length === 0}
                        >
                    ...
                    <div className="space-y-2">
                        ...
                        <Select
                            value={kotSettings.stockKotLanguage}
                            onValueChange={(val) => setKotSettings(prev => ({...prev, stockKotLanguage: val as AppLanguage}))}
                            disabled={isSettingDefault}
                        >
                 ...
                 <Button onClick={handleKOTSettingChange} disabled={isSettingDefault} size="sm">Save KOT Printer Settings</Button>
            </CardContent>
        </Card>
        ...
        <Card className="shadow-xl">
            ...
            <CardContent className="space-y-6">
                <div className="flex items-center justify-between p-3 border rounded-md">
                    ...
                    <Switch id="enableAutoKot" checked={kotSettings.enableAutoKotPrinting} onCheckedChange={(val) => setKotSettings(prev => ({...prev, enableAutoKotPrinting: val}))}/>
                </div>
                {kotSettings.enableAutoKotPrinting && (
                    <div className="p-4 border rounded-md bg-muted/50 space-y-4">
                        <div>
                            <Label>Print Mode</Label>
                            <RadioGroup value={kotSettings.autoKotPrintMode} onValueChange={(val) => setKotSettings(prev => ({...prev, autoKotPrintMode: val as 'immediate' | 'batch'}))} className="flex gap-4 mt-2">
                        ...
                        {kotSettings.autoKotPrintMode === 'batch' && (
                            <div>
                                <Label htmlFor="kotThreshold">Print KOT after N orders</Label>
                                <Input id="kotThreshold" type="number" min="2" value={kotSettings.kotBatchPrintThreshold} onChange={(e) => setKotSettings(prev => ({...prev, kotBatchPrintThreshold: Number(e.target.value)}))}/>
                ...
                <Button onClick={handleKOTSettingChange} disabled={isSettingDefault} size="sm">Save Auto-Print Settings</Button>
            </CardContent>
        </Card>
        </>
      )}
