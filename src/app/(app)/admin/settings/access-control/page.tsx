...
import { ArrowLeft, ShieldAlert, UserCog, ListChecks, Edit, Save, PlusCircle, Lock, Loader2, AlertTriangle } from "lucide-react"; // Added AlertTriangle
...
export default function AccessControlPage() {
  ...
  const managedRoles = useMemo(() => {
    ...
    return allRoleNames.map(name => {
      let description = "";
      const isCustom = !DEFAULT_USER_ROLES.includes(name);
      ...
      const updatedPermissions = prevPermissions.map(rp => {
        if (rp.roleName === roleName) {
          const newAllowedRoutes = checked
            ? [...rp.allowedRouteIds, routeId]
            : rp.allowedRouteIds.filter(id => id !== routeId);
          return { ...rp, allowedRouteIds: Array.from(new Set(newAllowedRoutes)) }; 
        }
        return rp;
      });
      if (!updatedPermissions.find(rp => rp.roleName === roleName)) {
  ...
  const handleCreateRole = () => {
    ...
    if (managedRoles.some(r => r.name === trimmedRoleName) || DEFAULT_USER_ROLES.includes(trimmedRoleName)) {
          ...
          {managedRoles.map(role => {
            ...
            <div key={role.name} className="space-y-3 p-4 border rounded-lg bg-card">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-3">
                {Object.entries(
                    ALL_APPLICATION_ROUTES.reduce((acc, route) => {
                    ...
                    <div key={groupName} className="col-span-1 sm:col-span-2 lg:col-span-1">
                        ...
                        <div key={route.id} className="flex items-center space-x-2 py-1">
                            ...
                            checked={isSuperAdminRole || currentRolePermissions?.allowedRouteIds.includes(route.id)}
