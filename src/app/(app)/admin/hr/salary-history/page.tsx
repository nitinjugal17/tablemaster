...
export default function SalaryHistoryPage() {
  ...
  const paymentDates = useMemo(() => payments.map(p => new Date(p.paymentDate)), [payments]);

  const filteredPayments = useMemo(() => {
    return payments.filter(p => {
      const matchesDate = !selectedDate || isSameDay(parseISO(p.paymentDate), selectedDate);
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {filteredPayments.map(p => (
                                    <TableRow key={p.id}>
                                        <TableCell className="text-xs">{format(parseISO(p.paymentDate), 'MMM d, yyyy')}</TableCell>
                                        <TableCell className="font-medium">{p.employeeName}</TableCell>
                                        <TableCell className="text-xs">{format(parseISO(p.periodFrom), 'dd/MM/yy')} - {format(parseISO(p.periodTo), 'dd/MM/yy')}</TableCell>
