...
export default function MenusManagementPage() {
  ...
  const handleSaveMenu = async (menuToSave: Menu) => {
    setIsSaving(true);
    let updatedMenus;
    const isNew = !menuToSave.id;
    
    // Assign a new ID if it's a new menu
    const finalMenu = isNew ? { ...menuToSave, id: crypto.randomUUID() } : menuToSave;

    if (isNew) {
      updatedMenus = [...menus, finalMenu];
    } else {
      updatedMenus = menus.map(m => (m.id === finalMenu.id ? finalMenu : m));
    }
                ...
                <Card key={menu.id} className="shadow-sm">
                    ...
                    <CardContent>
                         <p className="text-sm text-muted-foreground">
                            <strong>{Array.isArray(menu.menuItemIds) ? menu.menuItemIds.length : (menu.menuItemIds ? 1 : 0)}</strong> item(s) in this menu.
                         </p>
                    </CardContent>
                </Card>
              ))}
