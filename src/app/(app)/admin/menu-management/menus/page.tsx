...
import { getMenus, saveMenus, getMenuItems } from '@/app/actions/data-management-actions';
...
export default function MenusManagementPage() {
  ...
  const handleSaveMenu = (menuToSave: Menu) => {
    setMenus(prev => {
        const existingIndex = prev.findIndex(m => m.id === menuToSave.id);
        if (existingIndex > -1) {
            const updated = [...prev];
            updated[existingIndex] = menuToSave;
            return updated;
        }
        return [...prev, menuToSave];
    });
    setIsEditorOpen(false);
    setEditingMenu(undefined);
    toast({ title: "Menu Saved Locally", description: `Changes to "${menuToSave.name}" are local until you save all changes.`});
  };

  const handleDeleteMenu = (menuId: string) => {
    setMenus(prev => prev.filter(m => m.id !== menuId));
    toast({ title: "Menu Deleted Locally", description: `Menu marked for deletion. Save all changes to persist.`, variant: "destructive"});
  }
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>View, edit, or delete your current menus. Changes are temporary until saved.</CardDescription>
        </CardHeader>
        <CardContent>
          {menus.length > 0 ? (
                ...
                <Card key={menu.id} className="shadow-sm">
                            ...
                            <AlertDialog>
                                ...
                                <AlertDialogContent>
                                    ...
                                    <AlertDialogFooter>
                                        ...
                                        <AlertDialogAction onClick={() => handleDeleteMenu(menu.id)}>Delete Locally</AlertDialogAction>
                                    </AlertDialogFooter>
                                </AlertDialogContent>
                            </AlertDialog>
                        </div>
                    ...
                    <CardContent>
                         <p className="text-sm text-muted-foreground">
                            <strong>{menu.menuItemIds.length}</strong> item(s) in this menu.
                         </p>
                    </CardContent>
                </Card>
              ))}
