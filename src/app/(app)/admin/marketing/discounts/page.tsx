...
import { ArrowLeft, Tag, PlusCircle, Edit3, Trash2, Loader2, Save, MoreVertical, Percent, BadgeDollarSign, CalendarDays, CheckCircle, XCircle } from "lucide-react";
...
import { getDiscounts, saveDiscounts as saveDiscountsAction } from "@/app/actions/data-management-actions";
...
const DiscountEditor: React.FC<DiscountEditorProps> = ({ discount, onSave, onClose, existingCodes }) => {
  const form = useForm<DiscountFormValues>({
    resolver: zodResolver(discountCodeSchema.refine(data => {
      if (!discount?.id) { // New code
        return !existingCodes.includes(data.code.toUpperCase());
      }
      return !existingCodes.filter(c => c !== discount.code?.toUpperCase()).includes(data.code.toUpperCase());
    }, { message: "This discount code already exists.", path: ["code"] })),
    defaultValues: {
      id: discount?.id || "",
      code: discount?.code || "",
      ...
      validFrom: discount?.validFrom ? parseISO(discount.validFrom) : new Date(),
      validTo: discount?.validTo ? parseISO(discount.validTo) : new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // Default 7 days
          ...
          <FormField control={form.control} name="validFrom" render={({ field }) => (
            <FormItem className="flex flex-col"><FormLabel>Valid From *</FormLabel><Popover><PopoverTrigger asChild><FormControl>
                <Button variant="outline" className={cn("w-full pl-3 text-left font-normal", !field.value && "text-muted-foreground")}>
                  {field.value ? format(field.value, "PPP") : <span>Pick a date</span>} <CalendarDays className="ml-auto h-4 w-4 opacity-50" />
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        toast({ title: "Error", description: "Could not load discount codes.", variant: "destructive" });
      } finally {
        setIsLoading(false);
      }
    }
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          {isEditorOpen && (
            <DiscountEditor 
              ...
              existingCodes={editingDiscount?.id ? existingCodes.filter(c => c !== editingDiscount.code?.toUpperCase()) : existingCodes}
            />
          )}
        </DialogContent>
      </Dialog>
              ...
              <TableBody>
                {discounts.map((d) => (
                  <TableRow key={d.id}>
                    ...
                    <TableCell className="text-xs">
                      {d.minOrderAmount && d.minOrderAmount > 0 ? `${d.minOrderAmount} ${BASE_CURRENCY_CODE}` : 'None'}
                    </TableCell>
                    <TableCell>
