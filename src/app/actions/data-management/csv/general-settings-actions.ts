...
export async function getGeneralSettings(): Promise<InvoiceSetupSettings> {
  try {
    ...
    deserializedSettings.autoApproveNewOrders = String(settings.autoApproveNewOrders).toLowerCase() === 'true';
    deserializedSettings.autoApproveTableBookings = String(settings.autoApproveTableBookings).toLowerCase() === 'true';
    deserializedSettings.autoApproveRoomBookings = String(settings.autoApproveRoomBookings).toLowerCase() === 'true';
    deserializedSettings.loyaltyProgramEnabled = String(settings.loyaltyProgramEnabled).toLowerCase() === 'true';
    deserializedSettings.pointsPerCurrencyUnit = Number(settings.pointsPerCurrencyUnit) || defaultInvoiceSetupSettings.pointsPerCurrencyUnit;
    deserializedSettings.pointValueInCurrency = Number(settings.pointValueInCurrency) || defaultInvoiceSetupSettings.pointValueInCurrency;
...
export async function saveGeneralSettings(settings: InvoiceSetupSettings): Promise<{ success: boolean; message: string }> {
  ...
  try {
    const settingsForCsv = {
      ...
      autoApproveNewOrders: String(settings.autoApproveNewOrders || false),
      autoApproveTableBookings: String(settings.autoApproveTableBookings || false),
      autoApproveRoomBookings: String(settings.autoApproveRoomBookings || false),
      loyaltyProgramEnabled: String(settings.loyaltyProgramEnabled || false),
      pointsPerCurrencyUnit: Number(settings.pointsPerCurrencyUnit) || 1,
      pointValueInCurrency: Number(settings.pointValueInCurrency) || 0.1,
    };
  ...
  try {
    ...
    for (const key of Object.keys(defaultInvoiceSetupSettings) as Array<keyof InvoiceSetupSettings>) {
        if (rawSettings.hasOwnProperty(key)) {
            if (key === 'gstPercentage' || key === 'vatPercentage' || key === 'cessPercentage' || key === 'autoLogoutTimeoutMinutes' || key === 'dailyRevenueThreshold' || key === 'employeeBonusAmount' || key === 'bonusPercentageAboveThreshold' || key === 'pointsPerCurrencyUnit' || key === 'pointValueInCurrency') {
            ...
            } else if (key === 'autoGenerateInvoiceFooterQuote' || key === 'enableAutomatedExpenseInventoryReport' || key === 'showCalculatedCostOnInvoiceAdmin' || key === 'showNutritionalInfoOnInvoice' || key === 'autoApproveNewOrders' || key === 'autoApproveTableBookings' || key === 'autoApproveRoomBookings' || key === 'loyaltyProgramEnabled') {
    ...
    if (finalSettings.autoApproveNewOrders === undefined) finalSettings.autoApproveNewOrders = false;
    if (finalSettings.autoApproveTableBookings === undefined) finalSettings.autoApproveTableBookings = false;
    if (finalSettings.autoApproveRoomBookings === undefined) finalSettings.autoApproveRoomBookings = false;
    if (finalSettings.loyaltyProgramEnabled === undefined) finalSettings.loyaltyProgramEnabled = false;
    if (finalSettings.pointsPerCurrencyUnit === undefined) finalSettings.pointsPerCurrencyUnit = 1;
    if (finalSettings.pointValueInCurrency === undefined) finalSettings.pointValueInCurrency = 0.1;
